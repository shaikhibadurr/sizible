...
export default function Sizer() {
  ...
  const [Height, HeightBorder] = useHoverComponents('Height', { inputTop: 0, selectArr: NeckList,right:0 }, { top: 240, left: -80, width: 510,rotate:90 },true);
  ...
  return (
    <Grid size={12}>
      <Typography variant="h4" sx={style.sizerTitle}>Set your perfect fit</Typography>
      <Stack sx={style.fullWidth} spacing={1} direction={'row'} justifyContent={'space-between'}>
        <Stack sx={style.relativePostion}>
          {Neck}
          {Shoulder}
          {Arms}
          {Legs}
        </Stack>
        <Stack sx={style.sizerImage} spacing={1}>
          {NeckBorder}
          {ShoulderBorder}
          {ArmsBorder}
          {LegsBorder}
